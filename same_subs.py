...
def speak_tts(text):
    ...
    subprocess.run(["sox",str(raw),"-r","16000","-c","1","-b","16","-e","signed-integer", str(final), "norm","-3"], check=True)
    if final16.exists(): final16.unlink(missing_ok=True)
...
def main():
    ...
    if mode == "add":
        if len(code) != 6: speak_tts("Invalid code."); return
        upsert_code(ext, code); speak_tts("Code saved. Thank you."); return

    if mode == "remove":
        if len(code) != 6: speak_tts("Invalid code."); return
        ok = remove_code(ext, code); speak_tts("Code removed." if ok else "That code was not found."); return

    if mode == "list":
        codes = list_codes(ext)
        if not codes: speak_tts("You are not subscribed to any codes.")
        else: speak_tts("You are subscribed to the following codes: " + ", ".join(codes))
        return
